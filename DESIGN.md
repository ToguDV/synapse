...
| Latte · error icon on panel              | 4.5:1  | Non-text |
| Mocha · error icon on panel              | 7.6:1  | AAA      |

### Iconography

Chrome icons are 2px line SVGs on a 24px viewBox. They stay neutral — inheriting the surrounding
text color — and only destructive actions carry a hue: the trash uses `--error`, resolved through
`--icon-error` and a `[data-tone]` rule, so Tailwind utilities can still override. No decorative
rainbow.

Sizes run 12–17px. Menu tiles are 28px (`--panel` with a hairline); the destructive item tints its
tile with the error hue at 12% fill / 26% border.
