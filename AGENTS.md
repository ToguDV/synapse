...
- E2E (Playwright, headless, sin Electron): sirve solo el renderer en el contenedor y lanza los runners desde el host. Los runners resuelven Playwright con `e2e/playwright.cjs` (dep del proyecto → `SYNAPSE_PLAYWRIGHT` → caché de `npx`), así que ya no dependen de esta máquina:
  ...
  node e2e/mobile.e2e.cjs                                                           # 36 checks (drawer, sheets, gestos táctiles, swipe-close, idioma)
...
11. **Futuro** (post-MVP) — rich text inline (segmentos con marcas), tablas, gráficas, export MD, sync
     ...
     - UI móvil pendiente: multi-selección táctil (modo selección) e indent/outdent sin teclado.
       Hecho (2026-09-25): swipe-to-close en sheets (`Sheet.tsx`: agarre `[data-sheet-grabber]`,
       umbral 64px, se ignora con scroll; `shouldDismissSheet` testeable) y selector de idioma EN/ES
       (`shared/language.ts`, `store/localeStore.ts`, catálogo `locales/es.ts` con paridad tipada,
       setting `language` persistido, segmentado en el footer del sidebar). Tests: 240 unit
       (`language`, `sheet`, paridad `es` en `i18n`) y `e2e/mobile.e2e.cjs` 30→36 checks (C15–C20).
...
- **Puerto**: el servidor E2E de `tests/vite.e2e.config.ts` usa el 5174 (el dev normal usa 5173). Ojo: `ss` no es fiable en este contenedor (informa del puerto como libre mientras el server responde); comprobar con `curl`.
- **Entorno E2E sin root (`e2e/run.sh`)**: en un contenedor Linux pelado (sin root, sin systemd, sin fuentes del sistema) las suites fallan por dos motivos que no tienen que ver con el código. (1) Faltan las libs que enlaza Chromium: el navegador no arranca (`exitCode=127`, `error while loading shared libraries`). (2) **No hay fuentes**: Chromium arranca, pero mide el texto con **alto 0**, así que `waitForSelector('[data-block-id]')` espera 30 s a un elemento que existe y tiene contenido — el error dice "element is not visible" y despista por completo. `./e2e/run.sh doctor` diagnostica ambos (lanza Chromium como lo hacen las suites y mide el alto real de una línea de texto) y `./e2e/run.sh env` prepara `.cache/e2e/` descargando paquetes Debian con `apt-get download` + `dpkg -x` (sin privilegios) y escribiendo su propio `fonts.conf`; `./e2e/run.sh [suites…]` arranca el dev server, ejecuta y lo detiene. Las libs se detectan con `ldd`, pero hay módulos que Chromium carga con `dlopen` y no aparecen ahí (`libsoftokn3.so` de NSS: sin él aborta con `FATAL:crypto/nss_util.cc` si no se pasa `--password-store=basic`), por eso el script los copia aparte. En una máquina con libs y fuentes del sistema todo esto es un no-op.
- **Paralelismo**: las suites van **secuencialmente** (un navegador a la vez, ~350 MB). Lanzar los 11 runners en paralelo ahoga el dev server y produce fallos falsos por timeout de navegación.
- **Memoria (2026-09-25)**: esta máquina tiene ~2 GB de RAM; con builds de otros proyectos en marcha, correr las 11 suites (`run.sh` sin args) o dejar servidores colgados la deja sin memoria. Reglas: una suite por vez (`./e2e/run.sh mobile`), no arrancar vite manual con `nohup` (si se hace, matar el PID al terminar), y `run.sh` ya gestiona el ciclo del servidor (lo detiene solo). Antes de lanzar E2E, comprobar `free -m` y que no quede ningún `vite --config tests/vite.e2e.config.ts` previo (`ps aux | grep vite`).
